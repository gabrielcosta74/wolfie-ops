...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${env.supabaseUrl}/functions/v1/question-bank-profiler`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      workflow_code: "question-bank-profiler",
    }),
  });

  const payload = await response.json().catch(() => ({
    ok: false,
    message: "Invalid JSON response from question-bank-profiler",
  }));
