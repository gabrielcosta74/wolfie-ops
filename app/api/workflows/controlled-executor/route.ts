...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${env.supabaseUrl}/functions/v1/controlled-executor`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      workflow_code: "controlled-executor",
    }),
  });

  const payload = await response.json().catch(() => ({
    ok: false,
    message: "Invalid JSON response from controlled-executor",
  }));
