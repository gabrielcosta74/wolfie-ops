...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${env.supabaseUrl}/functions/v1/exercise-auditor`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      workflow_code: "exercise-auditor",
    }),
  });

  const payload = await response.json().catch(() => ({
    ok: false,
    message: "Invalid JSON response from exercise-auditor",
  }));
